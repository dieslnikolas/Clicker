...
Add-Line('using {0}.Controllers{1}{2}{3};' -f $data.Metadata.AppNamespace, ('.',('.{0}.'-f $data.Metadata.Modules))[![string]::IsNullOrEmpty($data.Metadata.Modules)], $data.Metadata.PluralName, (('.'+$data.Metadata.Prefix),'')[$data.Metadata.OperationType -eq 'BLANK'])
Add-Line('using {0}.Mvc.Common;'-f $data.Metadata.FrameworkNamespace, $data.Metadata.Modules, $data.Metadata.PluralName)
...
Add-Line('namespace {0}.Controllers' -f $data.Metadata.AppNamespace, ('.',('.{0}.'-f $data.Metadata.Modules))[![string]::IsNullOrEmpty($data.Metadata.Modules)], $data.Metadata.PluralName)
...
if ($data.Metadata.OperationType -ne 'BLANK') {
    ...
    if ($data.Metadata.OperationType -eq 'ALL') {
        ...
        Add-Line('        public ActionResult {2}({1}{0}FilterModel model)' -f $data.Metadata.Name, $data.Metadata.Prefix, ($data.Metadata.PrefixType+$data.Metadata.PrefixExtension))
        Add-Line('        {')
    ...
    elseif  ($data.Metadata.OperationType -in ('NEW')) {
        ...
        Add-Line('        {')
        Add-Line('            var handler = Handler.Get<{1}{0}Handler>().Handle(model);' -f $data.Metadata.Name, $data.Metadata.Prefix)
        Add-Line('            //if (handler.IsSuccess)')
        Add-Line('            //{')
        Add-Line('            //    TempData["Info"] = "{0}"' -f $data.Metadata.Prefix)
        Add-Line('            //{')
        Add-Line('            return AsView(handler, RedirectToAction("Index", "{0}"), {1}Model(model));' -f $data.Metadata.Name, $data.Metadata.Prefix)
        Add-Line('        }')
        Add-Line('')
    }
