...
if ($data.Metadata.OperationType -eq 'ALL') {
    ...
    Add-Line('        public ModelCoreResult<ICollection<{0}{1}OutputModel>> {1}({0}{1}InputModel input)' -f $data.Metadata.Name, $data.Metadata.Prefix)
    Add-Line('        {')
...
Add-Line('                    {')
    ...
    foreach ($column in $data.OutputColumns.GetEnumerator())
    {  
        $type = '{1}{0}' -f $column.Value.UtilsType, ('','Nullable')[$column.Value.IsNullable -and $column.Value.Type -NotIn ('string')]#[$column.Value.Type -in ('Guid', 'string', 'bool')]
        # todo typ
