...
$publishFolder = (Join-Path -Path $installFolderPath -ChildPath $part)
New-Item -ItemType Directory -Force -Path $publishFolder
$profileFile = Join-Path -Path $path -ChildPath ("{0}\Properties\PublishProfiles\{1}{2}.pubxml" -f $apiNamespace, $projectName, $part)
EditPublishFile $profileFile $publishFolder

# cesta k csproj
$csprojFile = Join-Path -Path $path -ChildPath ("{0}\{0}.csproj" -f $apiNamespace)
# sestaveni parametru pro build
$buildParams = "/p:DeployOnBuild=true /p:PublishProfile=""{0}"" /property:Configuration=Release" -f $profileFile
...
# cesta k csproj
$csprojFile = Join-Path -Path $path -ChildPath ("{0}\{0}.csproj" -f $serviceNamespace)
# sestaveni parametru pro build
$buildParams = "/property:Configuration=Release"
...
Copy-Item -Path "$path\$serviceNamespace\bin\release\*.exe" -Destination "$publishFolder"
Copy-Item -Path "$path\$serviceNamespace\bin\release\*.config" -Destination "$publishFolder"
Copy-Item -Path "$path\$serviceNamespace\bin\release\*.cmd" -Destination "$publishFolder"
Copy-Item -Path "$path\$serviceNamespace\bin\release\*.dll" -Destination "$publishFolder"

# upravit koncovky u configu na *.clear
Get-ChildItem -Path $publishFolder -Filter *.config | Rename-Item -NewName { $_.name -Replace '\.config$','.config.clear' } -Force
# upravit koncovky u cmd na *.clear
Get-ChildItem -Path $publishFolder -Filter *.cmd | Rename-Item -NewName { $_.name -Replace '\.cmd$','.cmd.clear' } -Force
