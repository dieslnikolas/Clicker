...
Add-Line('@{')
Add-Line('  ViewBag.Title = "Title";  //Resources.Dictionary.{0}{1}_All_Title; // {4}' -f ('',('.{0}_'-f $data.Metadata.Modules))[![string]::IsNullOrEmpty($data.Metadata.Modules)], $data.Metadata.PluralName, $data.Metadata.Prefix, $data.Metadata.Name, $data.Metadata.ProcedureDescription, $data.Metadata.AppNamespace)
Add-Line('}');
Add-Line('')
...
Add-Line('               @using (Html.BeginForm("{0}", "{1}", new {{ ID = Model.Filter.ID }} , FormMethod.Post))' -f $data.Metadata.Prefix, $data.Metadata.Name)
Add-Line('               {')
Add-Line('                  @Html.Partial("_{0}Filter", Model.Filter)' -f $data.Metadata.Prefix)
Add-Line('                  @Html.Partial("_{0}Items", Model.Items)' -f $data.Metadata.Prefix)
Add-Line('               }')
Add-Line('            </div>')
Add-Line('')
