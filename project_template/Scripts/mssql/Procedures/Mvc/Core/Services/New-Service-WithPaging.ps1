...
Add-Line('                    {')
    ...
    Add-Line('                    {')
    Add-Line('                        var item = new {0}{1}OutputItemModel();' -f $data.Metadata.Name, $data.Metadata.Prefix)
    Add-Line('')
    ...
    Add-Line('')
    Add-Line('                        result.Data.Items.Add(item);' -f $data.Metadata.Name, $data.Metadata.Prefix)
    Add-Line('                    }')
    Add-Line('')
    Add-Line('                    // Pagination')
    Add-Line('                    if (db.DataReader.NextResult())')
    Add-Line('                    {')
    Add-Line('                        db.Read();')
    Add-Line('                        int itemsCount = db.GetInt("ItemsCount");')
    Add-Line('                        if(input.PageSize.HasValue && input.Page.HasValue)')
    Add-Line('                        {')
    Add-Line('                            int totalPages = itemsCount / input.PageSize.Value;')
    Add-Line('                            if (itemsCount % input.PageSize.Value > 0)')
    Add-Line('                                totalPages ++;')
    Add-Line('')
    Add-Line('                            result.Data.TotalPages = totalPages;')
    Add-Line('                        }')
    Add-Line('                    }')
