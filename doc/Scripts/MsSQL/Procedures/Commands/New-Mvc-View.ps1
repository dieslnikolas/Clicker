...
if ($obj.Metadata.OperationType -eq 'NEW') {
    ...
    # list-filter
    $filepath = Join-Path -Path $path -ChildPath $('_{0}Filter.cshtml' -f $obj.Metadata.Prefix)
    ...
    # list-item
    $filepath = Join-Path -Path $path -ChildPath $('_{0}Items.cshtml' -f $obj.Metadata.Prefix)
