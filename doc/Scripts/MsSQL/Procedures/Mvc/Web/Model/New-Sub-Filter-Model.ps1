...
# hlavicka
Add-Line('namespace {0}.Controllers{1}{2}{3}' -f $data.Metadata.AppNamespace, ('.',('.{0}.'-f $data.Metadata.Modules))[![string]::IsNullOrEmpty($data.Metadata.Modules)], $data.Metadata.PluralName, (('.'+$data.Metadata.TableRelation),'')[$data.Metadata.OperationType -eq 'BLANK'])
...
{  
    ...
    if ($column.Value.TableName -eq $obj.Metadata.Table) {
        ...
        Add-Line('        //[Display(Name = nameof(Resources.Dictionary{0}{1}_{2}), ResourceType = typeof(Resources.Dictionary))]' -f ('.',('.{0}_'-f $data.Metadata.Modules))[![string]::IsNullOrEmpty($data.Metadata.Modules)], $data.Metadata.PluralName, $name)
        Add-Line('        public {0} ID_Parent {{ get; set; }}' -f $type, $column.Value.Name)
        Add-Line('')
    } else {
    ...
    if ($column.Value.Name -eq 'DisplayName') {
        ...
        Add-Line('        [DisplayName("{0}")]' -f $column.Value.Description)
        Add-Line('        //[Display(Name = nameof(Resources.Dictionary{0}{1}_{2}), ResourceType = typeof(Resources.Dictionary))]' -f ('.',('.{0}_'-f $data.Metadata.Modules))[![string]::IsNullOrEmpty($data.Metadata.Modules)], $data.Metadata.PluralName, $name)
        Add-Line('        public {0} Unlink{1} {{ get; set; }}' -f $type, $column.Value.Name)
        Add-Line('')
    }
}
