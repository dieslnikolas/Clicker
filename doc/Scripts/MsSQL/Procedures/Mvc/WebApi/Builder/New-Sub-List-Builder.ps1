...
{  
    if ($column.Value.Name -eq 'ID_Login' -or $column.Value.Name -eq 'ID') {
        ...
        Add-Line('                {1} = filter.{0},' -f $column.Value.Name, $column.Value.Name)
    }
}
Add-Line('            });')
...
foreach ($column in $columns)
{  
    Add-Line('                {0} = x.{1},' -f $column.Value.Name, $column.Value.Name)
}
Add-Line('            }).ToList();')
Add-Line('')
...
{  
    if ($column.Value.Name -eq 'ID_Login' -or $column.Value.Name -eq 'ID') {
        ...
        if ($column.Value.Name -eq 'DisplayName') {
            Add-Line('                {1} = filter.Unlink{0},' -f $column.Value.Name, $column.Value.Name)
        } else {
            Add-Line('                {1} = filter.{0},' -f $column.Value.Name, $column.Value.Name)
        }
    }
